 ...
 *
 * Builds a complete blink-style project (symbol + footprint + device + power
 * library, schematic placement, wires, net label, PCB footprints, tracks) with
 * the real scripts in a throwaway temp dir, then asserts the format
...
function main() {
  ...
  try {
    ...
    // ---- symbol library ----
    run([SCRIPTS + '/generate-symbol.js', 'from-pins', '--dir', proj, '--name', 'RES',
      '--title', 'Resistor', '--designator', 'R', '--pins', '1;2']);
    const res = JSON.parse(fs.readFileSync(j('library', 'RES.json'), 'utf8'));
    assert(res.kind === 'symbol' && res.symbolDoc.length > 0, 'generate-symbol stages a symbol entry');
    ...
    // ---- footprint library ----
    run([SCRIPTS + '/generate-footprint.js', 'from-pads', '--dir', proj, '--name', 'FP0402',
    ...
    const fp = JSON.parse(fs.readFileSync(j('library', 'FP0402.json'), 'utf8'));
    assert(fp.kind === 'footprint' && fp.footprintElems.pads.length === 2,
      'generate-footprint stages a footprint entry with pad elements');
    ...
    // ---- device + power library ----
    run([SCRIPTS + '/load-library.js', 'device', '--dir', proj, '--symbol', 'RES',
      '--footprint', 'FP0402', '--name', 'R0402', '--title', 'R0402']);
    const dev = JSON.parse(fs.readFileSync(j('library', 'R0402.json'), 'utf8'));
    assert(dev.kind === 'device' && dev.symbolUuid !== dev.footprintUuid && dev.footprintUuid !== dev.deviceUuid,
      'device entry links distinct symbol/footprint/device uuids');
    const devMeta = dev.deviceDoc.map(E.parseRecord).find((r) => r.type === 'META');
    assert(devMeta.body.attributes && devMeta.body.attributes.Designator === 'R?',
      'device doc META carries the Designator');
    run([SCRIPTS + '/load-library.js', 'power', '--dir', proj, '--net', 'VCC']);
    const pwr = JSON.parse(fs.readFileSync(j('library', 'VCC.json'), 'utf8'));
    const pwrMeta = pwr.powerDeviceDoc.map(E.parseRecord).find((r) => r.type === 'META');
    assert(pwr.kind === 'power' && pwrMeta.body.attributes.Designator === undefined,
      'power device META has no Designator key (matches the example)');
    assert(pwrMeta.body.attributes['Global Net Name'] === 'VCC',
      'power device META carries Global Net Name');
    const rList = run([SCRIPTS + '/load-library.js', 'list', '--dir', proj]);
    assert(/R0402/.test(rList.stdout) && /VCC/.test(rList.stdout), 'load-library list shows staged entries');

    // ---- port library ----
    run([SCRIPTS + '/load-library.js', 'port', '--dir', proj, '--net', 'SIG', '--name', 'PORT_SIG']);
    const port = JSON.parse(fs.readFileSync(j('library', 'PORT_SIG.json'), 'utf8'));
    const portSymMeta = port.portSymbolDoc.map(E.parseRecord).find((r) => r.type === 'META');
    assert(port.kind === 'port' && portSymMeta.body.docType === 19 && portSymMeta.body.title === 'Port-SIG',
      'port entry stages a docType-19 NetPort symbol titled Port-SIG');
    ...
    run([SCRIPTS + '/add-symbol.js', '--dir', proj, '--sch', 'Schematic1', '--sheet', 'P1',
      '--lib', 'R0402', '--x', '300', '--y', '-440', '--rotation', '90']);
    run([SCRIPTS + '/add-power.js', '--dir', proj, '--sch', 'Schematic1', '--sheet', 'P1',
      '--lib', 'VCC', '--x', '300', '--y', '-500']);
    const sch = mainDocs(j('sch', 'Schematic1', 'P1.esch2'));
    const symUuids = new Set(sch.docs.filter((d) => d.docType === 'SYMBOL').map((d) => d.uuid));
    const devUuids = new Set(sch.docs.filter((d) => d.docType === 'DEVICE').map((d) => d.uuid));
    assert(symUuids.has(dev.symbolUuid), 'add-symbol embeds the SYMBOL doc');
    assert(devUuids.has(dev.deviceUuid), 'add-symbol embeds the DEVICE doc');
    // locate the R0402 component by its DeviceName uuid (the first COMPONENT
    // in the main doc is the A4 frame)
    const comp = sch.main.records.find((r) => r.type === 'COMPONENT' && r.body.attrs
      && r.body.attrs.DeviceName && JSON.parse(r.body.attrs.DeviceName).uuid === dev.deviceUuid);
    ...
    assert(symAttr && symAttr.body.parentId === comp.head.id && symUuids.has(symAttr.body.value),
      'Symbol attr links COMPONENT to the embedded SYMBOL doc');
    assert(devAttr && devAttr.body.parentId === comp.head.id && devUuids.has(devAttr.body.value),
    ...
    assert(desig && desig.body.parentId === comp.head.id, 'Designator attr parentId == COMPONENT id');
    ...
    const pwrComp = sch.main.records.filter((r) => r.type === 'COMPONENT')
      .find((r) => r.body.attrs && r.body.attrs.DeviceName === null);
    assert(pwrComp && pwrComp.body.attrs.pinClass === undefined,
      'power COMPONENT omits pinClass keys (matches the example)');
    assert(sch.main.records.some((r) => r.type === 'ATTR' && r.body.key === 'Global Net Name'
      && r.body.parentId === pwrComp.head.id && r.body.value === 'VCC'),
      'Global Net Name attr links the power COMPONENT to VCC');
    ...
    // ---- port + text + shapes on the sheet ----
    run([SCRIPTS + '/add-port.js', '--dir', proj, '--sch', 'Schematic1', '--sheet', 'P1',
      '--lib', 'PORT_SIG', '--x', '460', '--y', '-440']);
    const schP = mainDocs(j('sch', 'Schematic1', 'P1.esch2'));
    const symUuidsNow = new Set(schP.docs.filter((d) => d.docType === 'SYMBOL').map((d) => d.uuid));
    const portGnn = schP.main.records.find((a) => a.type === 'ATTR'
      && a.body.key === 'Global Net Name' && a.body.value === 'SIG');
    assert(portGnn, 'add-port writes a Global Net Name attr for SIG');
    ...
    assert(portSymAttr && symUuidsNow.has(portSymAttr.body.value),
      'port Symbol attr resolves to the embedded docType-19 SYMBOL doc');
    ...
    // ---- PCB placement ----
    run([SCRIPTS + '/add-footprint.js', '--dir', proj, '--pcb', 'PCB1', '--lib', 'R0402',
    ...
    assert(fpUuids.has(dev.footprintUuid), 'add-footprint embeds the FOOTPRINT doc');
