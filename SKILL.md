---
name: easyeda-eprj3
description: Create, edit, and convert EasyEDA Pro folder-based .eprj3 PCB projects. Use this skill whenever the user wants to author a schematic/PCB in EasyEDA Pro offline-client format from AI prompts, convert KiCad projects, ingest an EasyEDA library (.elibz2), or run validation. Drives the Node.js scripts under scripts/.
---
...
- Convert an existing **KiCad** project (`.kicad_pro`, `.kicad_sch`, `.kicad_pcb`) into eprj3.
- Import symbols / footprints from a local **EasyEDA library** (`.elibz2`) or a **KiCad library** (`.kicad_sym`, `.kicad_pcb`).
...
5. **Library source**:
   - If the user provides a path to a `.elibz2` file/directory or a KiCad library, **use it**.
...
4a. (Local lib)  node scripts/load-library.js import --dir <dir> --format elibz2|kicad --library <path> --component <name>
...
9.  node scripts/set-refdes.js renumber ...   (auto number R*, C*, U*, D*)
10. node scripts/validate.js check --dir <dir> [--fix]   (always run this)
11. If validation warns/errors, fix and re-run until clean.
12. node scripts/open.js open --dir <dir>     (launch EasyEDA Pro / LCEDA Pro offline client)
```

Step 10–11 is a loop: run, fix, run, fix — until the validator reports `0 errors, 0 warnings` (or the user accepts the warnings in `--strict` mode).
...
| `scripts/load-library.js` | Pull a component out of an `.elibz2` or KiCad library. |
...
| `scripts/convert-kicad.js` | Convert a whole KiCad project to eprj3 (best effort). |
...
The lower-level helpers live in `scripts/lib/` (`eprj3.js`, `kicad.js`, `elibz2.js`, `kicad-to-eprj3.js`, `utils.js`).
...
- After step 12, summarize the result as a single line: `<name>.eprj3 at <dir> — N components, M wires, validated, opened in EasyEDA Pro.`
...
- The KiCad converter handles `.kicad_sch` and `.kicad_pcb` primitives only. SPICE models, custom rules, and 3D models are not migrated.
- `load-library.js` requires the `yauzl` package when ingesting zip-based `.elibz2` archives. Run `npm install yauzl --no-save` first if the user supplies a zip library.